...
macOS：解压到可写的文件夹，双击 **安装并启动.command**。首次运行会检测 Node.js；缺少兼容版本时，从 nodejs.org 下载本项目专用的 Node 并校验 SHA-256，再安装固定版本的组件，最后打开浏览器。已有兼容环境会直接复用，不需要另装全局 GMGN CLI。

Windows：先从 nodejs.org 安装兼容的 Node.js。首次验收请双击 **TEST-WINDOWS.bat**，测试结束后在命令窗口按 `Ctrl+C` 完全停止；确认无误后日常可使用 **START-WINDOWS.bat**。这些入口尚待 Windows 10/11 实机验收，因此当前不能视为正式支持。Linux 可安装兼容 Node.js 后运行 `npm run setup` 与 `npm run open`。

页面打开后点击 **首次使用 / 创建 API**。每位使用者、每次创建新的 GMGN API，都必须完成 GMGN 要求的 Agent 密钥配对步骤；Meme雷达开源版会在本机替用户完成密钥生成，因此不必另外安装 Agent 或输入命令。复制页面显示的公钥并打开 GMGN 创建页，只开启 **允许读取**，务必关闭 **允许交易**；创建完成后把 API Key 粘贴回雷达并点击 **✓**。服务只验证该 Key 的读取权限，不会调用关注钱包或交易接口；成功后才保存 Key 并安排扫描。验证失败不会替换原有连接；未完成的公钥可以继续使用，也可以明确点击重新生成。

以后继续双击同一个入口即可；Key 保存在本机，重启后不用重复填写。扫描到的代币可点击官网、GMGN 或 X 链接人工查看。
